...
**Architectural rationale:** This project chose Option 1 (monolith) over microservices to simplify deployment, reduce operational overhead, and enable rapid iteration. The monolithic approach is well-suited for the current scale and team size, and background job processing is handled via BullMQ workers within the same codebase.
...
- Params: `{ params }: { params: { productId: string } }` (params is synchronous)
