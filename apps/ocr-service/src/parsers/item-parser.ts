...
const SKIP_PATTERNS = [
  ...
  /\b(billa|spar|hofer|lidl|penny|interspar|merkur|m-preis|mpreis|unimarkt|nah\s*&\s*frisch|walmart|target|costco|kroger|safeway|trader\s*joe'?s?|whole\s*foods)\b/i, // Store names (Austrian & US) - production filtering
  ...
  /^[a-z .'\\-]+,?\s+[a-z]{2}\s+\d{5}/i, // City, State ZIP (US format, flexible city)
